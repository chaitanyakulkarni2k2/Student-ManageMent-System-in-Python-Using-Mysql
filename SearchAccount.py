...
def search_user_account():
    ...
    def search_account():
        ...
        if acc_no in numb_list:
            ...
            # Date of Birth
            DOB_label = Label(right_frame, text="Prn No:-  {}".format(str(rec_list[4])),
                              bg="#7209b7", fg="#ced4da", font='Times 30 bold', anchor=W)
            DOB_label.pack(fill=X, pady=2, padx=2)

            # Account number
            ac_no_label = Label(right_frame, text="4th Y Marks:-  {}".format(rec_list[1]),
                                bg="#ced4da", fg="#7209b7", font='Times 30 bold', anchor=W)
            ac_no_label.pack(fill=X, pady=2, padx=2)

            # Contact Number
            contact_label = Label(right_frame, text="12th Marks:- {}".format(rec_list[7]),
                                  bg="#7209b7", fg="#ced4da", font='Times 30 bold', anchor=W)
            contact_label.pack(fill=X, pady=2, padx=2)

            # Gender
            gender_label = Label(right_frame, text="Email:- {}".format(rec_list[2]),
                                 bg="#ced4da", fg="#7209b7", font='Times 30 bold', anchor=W)
            gender_label.pack(fill=X, pady=2, padx=2)

            # aadhar
            aadhar_label = Label(right_frame, text="Ist Y Marks:- {}".format(rec_list[3]),
                                 bg="#7209b7", fg="#ced4da", font='Times 30 bold', anchor=W)
            aadhar_label.pack(fill=X, pady=2, padx=2)

            # Account Type
            ac_type_label = Label(right_frame, text="2th Y Marks:-{}".format(rec_list[6]),
                                  bg="#ced4da", fg="#7209b7", font='Times 30 bold', anchor=W)
            ac_type_label.pack(fill=X, pady=2, padx=2)

            # address Type
            address_label = Label(right_frame, text="3rd Marks:- {}".format(rec_list[5]),
                                  bg="#7209b7", fg="#ced4da", font='Times 30 bold', anchor=W)
    ...
    account_number = Label(base_frame, text="Enter Prn No:",
