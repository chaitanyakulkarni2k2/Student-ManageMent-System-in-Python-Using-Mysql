...
db = mysql.connector.connect(host="localhost",user="root",passwd="chaitanya123",database="project_db")
mycur = db.cursor()
...
def login():
    ...
    Label(root2, text="").pack()
    Button(root2, text="Log-In", bg="green",command=login_varify).pack()
    Label(root2, text="")
...
def logged():
    ...
    logg = Toplevel(root2)
    logg.title("Welcome")
    logg.geometry("800x300")
    Label(logg, text="Welcome {}! ".format(username_varify.get()), fg="Blue", font="bold").pack()
    Label(logg, text="{} And Now You can acess this student management system ".format(username_varify.get()), fg="green", font="bold").pack()
    Label(logg, text=" Now {}, You have to close all the windows ".format(username_varify.get()), fg="red", font="bold").pack()
    Label(logg, text=" Then {}, You can automatically reach to the Landing Page  ".format(username_varify.get()), fg="brown", font="bold").pack()
    Label(logg, text="  As you can closes all the windows".format(username_varify.get()), fg="brown", font="bold").pack()
    Label(logg, text="Thank u".format(username_varify.get()), fg="blue", font="bold").pack()
...
def login_varify():
    ...
    sql = "select * from login where  u_name = %s and password = %s"
...
def main_screen():
    ...
    Label(root,text="Welcome to Chaitanya Kulkarni",font="bold",bg="grey",fg="black",width=300).pack()
    Label(root,text="").pack()
    Button(root,text="Log-IN",width="8",height="1",bg="green",font="bold",command=login).pack()
    Label(root,text="").pack()
    Button(root, text="Registration",height="1",width="15",bg="green",font="bold",command=registration).pack()
    Label(root,text="").pack()
    Label(root,text="").pack()
    Label(root,text="Developed By Chaitanya Kulkarni").pack()
